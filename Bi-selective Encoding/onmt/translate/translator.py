...
class Translator(object):
    ...
    def translate(self,
        ...
        data = inputters.build_dataset(self.fields,
                                       ...
                                       template_iter=template_data_iter,
    ...
    def _translate_batch(self, batch, data):
        ...
        template = inputters.make_features(batch, 'template', data_type)
        template_lengths = None
        if data_type == 'text':
            _, src_lengths = batch.src
            _,template_lengths=batch.template

        enc_states, memory_bank = self.model.encoder(src, template,src_lengths,template_lengths)
        dec_states = self.model.decoder.init_decoder_state(
    ...
    def _run_target(self, batch, data):
        ...
        if data_type == 'text':
            _, src_lengths = batch.src
            _, template_lenghts=batch.template
        else:
            src_lengths = None
        src = inputters.make_features(batch, 'src', data_type)
        ...
        #  (1) run the encoder on the src
        enc_states, memory_bank = self.model.encoder(src, template,src_lengths,template_lenghts)
        dec_states = \
        ...
        dec_out, _, attens = self.model.decoder(
